...
export function createDialogueWriter(genre) {
  ...
  const writer = (hero, villain) => {
    if(hero === null || hero === undefined || hero.trim() === "") return "..."
    if(villain === null || villain === undefined || villain.trim() === "") return "..."
...
export function createTicketPricer(basePrice) {
  ...
  const calculetTicketPrice = (seatType, isWeekend = false) =>{
  ...
  return calculetTicketPrice
}

export function createRatingCalculator(weights) {
  if(typeof weights !== "object" || weights === null) return null

  const averageWeights = (scores) => {
  const keysArray = Object.keys(scores)

  const reduceKeyArray = keysArray.reduce((total, key) =>{
  ...
  return Number(reduceKeyArray.toFixed(1))
  }
