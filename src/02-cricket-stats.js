...
export const getPlayerCard = (player) => {
  if( player === null || player === undefined || player.name === undefined || player.name === null) return null
  
  const calculatedStrikeRate = calcStrikeRate(player.runs, player.balls)
  const calculatedEconomy = calcEconomy(player.runsConceded, player.overs)
  const calculatedBattingAvg = calcBattingAvg(player.totalRuns, player.innings, player.notOuts)

  const allRounderStatus = isAllRounder(player)

  return {
    name: player.name,
    strikeRate: calculatedStrikeRate,
    economy: calculatedEconomy,
    battingAvg: calculatedBattingAvg,
